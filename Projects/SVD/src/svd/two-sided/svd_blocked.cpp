...
std::size_t colbnsvd(struct matrix_t Amat, struct matrix_t Bmat, struct matrix_t Umat, struct matrix_t Vmat,
    std::size_t block_size, bool vectorization, double* Time, struct string_t errors) {
 ...
 * @param matrix_t Umat матрица левых сингулярных векторов
 * @param matrix_t Vmat матрица правых сингулярных векторов
 * @param std::size_t ThreadsNum число потоков параллельного исполнения
 * @param bool vectorization маркер используемой векторизации кода
 * @param double* Time время затраченное на вычисления
 * @param struct string_t errors информация об ошибке
 * @return std::size_t sweeps число разверток методом Якоби
 **/
 ...
 * @param matrix_t Vmat матрица правых сингулярных векторов
 * @param std::size_t ThreadsNum число потоков параллельного исполнения
 * @param std::size_t l1_max_elements сколько элементов (типа double) помещается в L1 кэш
 * @param bool vectorization маркер используемой векторизации кода
 * @param double* Time время затраченное на вычисления
 * @param struct string_t errors информация об ошибке
 * @return std::size_t sweeps число разверток методом Якоби
...
std::size_t rrbnsvd_parallel_cache(struct matrix_t Amat, struct matrix_t Bmat, struct matrix_t Umat, struct matrix_t Vmat, std::size_t l1_kb_size, bool vectorization, double* Time, struct string_t errors)
{
	...
	std::size_t max_sweeps = 40; //максимальное число повторения итераций
	std::size_t sweeps = 0;  //число повторений цикла развертки
	std::size_t block_size; //размер блока для подматрицы (всего subproblem_blocks на одну подматрицу)
	const double tol = 1e-10;  //точность предела сходимости
	std::size_t n = Amat.rows; //размер матрицы
	double norm = 0.0;      //норма Фробениуса матрицы B
	double off_norm = 0.0;  //норма Фробениуса только недиагональных элементов матрицы B
