...
std::size_t svd_subprocedure(struct matrix_t Bmat, struct matrix_t Umat, struct matrix_t Vmat) {
    ...
    double* U = Umat.ptr;
    double* V = Vmat.ptr;
    double norm = 0.0;      //норма ‘робениуса матрицы B
    double off_norm = 0.0;  //норма ‘робениуса только недиагональных элементов матрицы B

    //матрицы U и V инициализируютс€ как единичные матрицы
    matrix_identity(Umat);
    matrix_identity(Vmat);
    //вычисление нормы ‘робениуса, и нормы недиагональных элементов
    ...
    while (sqrt(off_norm) > tol * sqrt(norm)) {
        for (std::size_t i = 0; i < n - 1; ++i) {
            for (std::size_t j = i + 1; j < n; ++j) {
                ...
                //вычислить коэффициенты cos sin дл€ элемента bij
